...
double laplace_solve(double *A, int ROWS, int COLS, int numDev) {
  ...
  dim3 gridSize_boundary( ceil((double)(COLS)/blockSize_boundary.x));
  ...
  for(int i=0; i<nIters; i++)
  {
    for(int d=0;d<numDev;d++) {
      ...
      //Exterior Low
      simpleLaplaceIter_kernel<<<gridSize_boundary,blockSize_boundary>>>(1,COLS,d_Ain[d],d_Aout[d]);
     
      //Exterior High
      simpleLaplaceIter_kernel<<<gridSize_boundary,blockSize_boundary>>>(1,COLS,d_Ain[d]+IDX(rows-1,0,LDA),d_Aout[d]+IDX(rows-1,0,LDA));

      //Interior
      simpleLaplaceIter_kernel<<<gridSize,blockSize>>>(rows-2,COLS,d_Ain[d]+IDX(1,0,LDA),d_Aout[d]+IDX(1,0,LDA));
      cudaCheckError();
    }
